...
func TestApplyDeleteSpansBuffersUncovered(t *testing.T) {
	...
	docB.store.mu.Lock()
	docB.store.applyDeleteSpans(newTransaction(), []deleteSpan{{client: 1, clock: 1, length: 2}})
	...
	docB.store.mu.Lock()
	docB.store.applyDeleteSpans(newTransaction(), nil)
...
func TestApplyDeleteSpansConverges(t *testing.T) {
	...
	docB.store.mu.Lock()
	docB.store.applyDeleteSpans(newTransaction(), []deleteSpan{{client: 1, clock: 5, length: 6}})
	docB.store.mu.Unlock()
...
func TestApplyDeleteSpansIdempotentAcrossCalls(t *testing.T) {
	...
	doc.store.mu.Lock()
	doc.store.applyDeleteSpans(newTransaction(), span)
	doc.store.applyDeleteSpans(newTransaction(), span)
	doc.store.mu.Unlock()
