...
func applyUpdateTo(structs []updateItem) string {
	...
	s.applyUpdate(newTransaction(), u.structs)
...
func TestApplyUpdatePendingDrain(t *testing.T) {
	...
	s.applyUpdate(newTransaction(), []updateItem{i})
	...
	s.applyUpdate(newTransaction(), []updateItem{h})
...
func TestApplyUpdateOwnPredecessorGate(t *testing.T) {
	...
	s := newStore(0)
	s.applyUpdate(newTransaction(), []updateItem{h, x, k})
...
func TestApplyUpdateDuplicatesDropped(t *testing.T) {
	...
	s := newStore(0)
	s.applyUpdate(newTransaction(), []updateItem{h, i, h, i})
	s.applyUpdate(newTransaction(), []updateItem{h, i})
	if got := s.getOrCreateYType("text").Text(); got != "HI" {
		t.Fatalf("expected HI, got %q", got)
	}
	...
	s2.applyUpdate(newTransaction(), []updateItem{orphan})
	s2.applyUpdate(newTransaction(), []updateItem{orphan})
	if len(s2.pending) != 1 {
...
func TestApplyUpdateUnresolvableStaysPending(t *testing.T) {
	...
	s.applyUpdate(newTransaction(), []updateItem{ghostDep})
	s.applyUpdate(newTransaction(), []updateItem{h})
...
func TestApplyUpdateFragmentedDeliveryConverges(t *testing.T) {
	...
	for oi, ord := range orders {
		s := newStore(0)
		for _, ui := range ord {
			s.applyUpdate(newTransaction(), []updateItem{ui})
		}
...
func TestApplyUpdateMultipleRoots(t *testing.T) {
	...
	s.applyUpdate(newTransaction(), []updateItem{h, i, b})
