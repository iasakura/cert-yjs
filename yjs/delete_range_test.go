...
func TestDeleteRangeWholeRun(t *testing.T) {
	...
	doc.store.mu.Lock()
	doc.store.deleteRange(newTransaction(), 1, 1, 3) // clocks 1..3 = "ell"
	doc.store.mu.Unlock()
...
func TestDeleteRangeIdempotent(t *testing.T) {
	...
	doc.store.mu.Lock()
	doc.store.deleteRange(newTransaction(), 1, 0, 2)
	doc.store.deleteRange(newTransaction(), 1, 0, 2) // again: no double length shrink
	doc.store.mu.Unlock()
...
func TestDeleteRangeSkipsUnintegrated(t *testing.T) {
	...
	doc.store.deleteRange(newTransaction(), 1, 0, 5)
	// a client with no items at all
	doc.store.deleteRange(newTransaction(), 7, 0, 3)
	doc.store.mu.Unlock()
...
func TestDeleteRangeRemoteConverges(t *testing.T) {
	...
	docA.store.mu.Lock()
	docA.store.deleteRange(newTransaction(), 1, 5, 6) // " world"
	docA.store.mu.Unlock()

	docB.store.mu.Lock()
	docB.store.deleteRange(newTransaction(), 1, 5, 6)
