...
Proof.
  ...
  { (* ---- out-of-range: index past the visible length, nothing inserted. ---- *)
    wp_auto.
    iAssert (own_ytype_runs tv.(yjs.Text.inner') (DfracOwn 1) ls0 (MkTypeModel runs0 ts.(ty_arr)))
  ...
  wp_auto.
  iDestruct (own_store_runs_ytype_acc tv.(yjs.Text.store') (MkStoreStateRuns client k locs0 p0 bind pend pdel) tv.(yjs.Text.inner') ls0 (MkTypeModel runs0 ts.(ty_arr)) Hl0 Hp0 with "Hruns") as "[Hyt Hytback]".
  wp_apply (wp_yType__findPos tv.(yjs.Text.inner') (DfracOwn 1) ls0 (MkTypeModel runs0 ts.(ty_arr)) idx with "[$Hyt]").
  ...
  have Harr1 : ts.(ty_arr) = runs_flatten runs1 := Harrs1 _ _ Hp1.
  iDestruct (own_store_runs_arr_inv with "Hruns") as %Harrinvs1.
  have Hinvarr : YjsArrInvariant ts.(ty_arr) := Harrinvs1 _ _ Hp1.
  ...
  { move=> r Hr. apply Hwfall1. apply elem_of_all_runs.
    exists tv.(yjs.Text.inner'), (MkTypeModel runs1 ts.(ty_arr)). split; [exact Hp1 | exact Hr]. }
  ...
  iAssert (∃ (j : nat) (arr : list (YjsItem A)) (locsj : gmap loc (list loc)) (pj : pool)
    ...
    "Hlk" ∷ own_wlock γs ∗
    "Hdelete_set" ∷ own_delete_set γs m (all_cells (<[tv.(yjs.Text.inner') := MkTypeState (cells_of_locs_runs tv.(yjs.Text.inner') lsj runsj) arr]> types)) ∗
  ...
  { iExists 0%nat, ts.(ty_arr), locs1, p1, ls1, runs1, [], h.
  ...
  { (* loop body: integrate the [j]-th character *)
    ...
    { have -> : in_id input = MkYjsId (uint.nat client) (uint.nat k + j)%nat.
      ...
      - rewrite (Hdompj q Hne) /p0 /pool_of lookup_fmap in Hq.
        destruct (types !! q) as [ts'|] eqn:Hts'; [| discriminate]. injection Hq as <-. simpl in Hx.
        have := Hctr _ _ x Hts' Hx Hcx. lia. }
    ...
    have Hcells' : cells_of_locs_runs tv.(yjs.Text.inner') ls' runs' = take (p1i + j) cellsj ++ c :: drop (p1i + j) cellsj.
    { rewrite Hls'eq Hruns'eq /integrate_locs
        (cells_of_locs_runs_splice tv.(yjs.Text.inner') lsj runsj (p1i + j) oL2 (MkItemRun [newItem] false) Hlslj) //. }
    have Hpermc : take (p1i + j) cellsj ++ c :: drop (p1i + j) cellsj ≡ₚ cellsj ++ [c].
    { rewrite (Permutation_cons_append (drop (p1i + j) cellsj) c) app_assoc take_drop //. }
    have Hlookj : (<[tv.(yjs.Text.inner') := MkTypeState cellsj arr]> types) !! tv.(yjs.Text.inner') = Some (MkTypeState cellsj arr)
      by apply lookup_insert_eq.
    have Hac_ds : all_cells (<[tv.(yjs.Text.inner') := MkTypeState (cells_of_locs_runs tv.(yjs.Text.inner') ls' runs') arr']> types)
                ≡ₚ all_cells (<[tv.(yjs.Text.inner') := MkTypeState cellsj arr]> types) ++ [c].
    { rewrite Hcells'.
      have H := all_cells_insert_snoc (<[tv.(yjs.Text.inner') := MkTypeState cellsj arr]> types) tv.(yjs.Text.inner') cellsj arr
                  (take (p1i + j) cellsj ++ c :: drop (p1i + j) cellsj) arr' c Hlookj Hpermc.
      rewrite insert_insert in H. case_decide as Hd; [exact H | congruence]. }
    ...
    iDestruct (own_delete_set_snoc γs m _ _ c Hac_ds Hfresh_ds with "Hdelete_set") as "Hdelete_set".
    wp_for_post.
  ...
  iDestruct (own_delete_set_insert γs m (all_cells types') (RootId name) arr with "Hdelete_set") as "Hdelete_set".
  { move=> x Hx.
    rewrite Hmt in Hx.
  ...
  have Hregmodel_close : registry_models (<[RootId name := arr]> m) bind types'.
  ...
  have Hctr_close : ∀ parent' ts' x, types' !! parent' = Some ts' → x ∈ ty_arr ts' →
  ...
  { intros parent' ts' x Hlook Hxin Hxc. rewrite Hk'val.
      ...
      + rewrite lookup_insert_eq in Hlook. injection Hlook as <-. simpl in Hxin. exact (Hctrj x Hxin Hxc).
