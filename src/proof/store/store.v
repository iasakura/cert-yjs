(** Facade for the [store] proofs. One Go file ([yjs/store.go]), layered: the
    ...
    ([Integrate], [GetNode], [splitNode], [repair], [applyUpdate],
    [deleteRange]). Downstream
    files Require THIS module only; the split is an internal build-time
    concern. *)
From New.proof.store Require Export model value heap wp_private Integrate GetNode splitNode repair applyUpdate deleteRange.
