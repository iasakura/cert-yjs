...
Proof.
  ...
  iDestruct "Hrest" as (locs p m deleted) "[>Hstore Hreg]".
  ...
  iExists (uint.nat client), h, m, pend, deleted. iFrame "Hreg".
...
Proof.
  ...
  - iEval (cbn [tie_body]) in "Hbody".
    ...
    iDestruct (own_store_store_slices with "HR") as (client k items_mref types_mref deletedSetVal pend_sl pdel_sl locs' p' bind pdel delete_set) "(%Hc & %Hdel & Hexcl & Hro)".
    ...
    { iExists (RLocked 0). iFrame "Hrl0". iEval (cbn [tie_body]). iFrame "Hrauth Htoks0 Hwl".
      iExists locs', p', m, deleted. iFrame "Hreg". rewrite /tie_store.
...
Proof.
  ...
  iDestruct "Hrest" as (locs_i p_i m deleted) "[>Hstore Hreg]".
  iEval (rewrite /tie_store) in "Hstore".
  ...
  { iExists (RLocked n). iFrame "Hrln". iEval (cbn [tie_body]). iFrame "Hrauth Hmaxn Hwl".
    iExists locs_i, p_i, m, deleted. iFrame "Hreg". rewrite /tie_store.
    iExists client, k, items_mref, types_mref, deletedSetVal, pend_sl, pdel_sl, bind, h, pend, pdel, delete_set.
